...
T8:	bne $s1,8, T8no	# if s1 = 7, do these things. Else go to T7no
...
Insert_end:
	...
	goLastItem :
	lw $t7, 0($t0) # next of current node
	beq $t7, $0, doneGoLastItem
	add $t0, $0, $t7
			
	j goLastItem
	
	doneGoLastItem :
	li $a0, 8	# get 8 bytes of heap from OS
	li $v0, 9	# syscall for sbrk (dynamic memory allocation)
	syscall
	
	add $t6, $0, $v0 #Pointer to allocated memory
	sw $t6, 0($t0) #next to the last one
	sw $0, 0($t6) #put NULL
	sw $t1, 4($t6) #put value
	add $v0, $0, 0 #success
...
Insert_n:
	...
	goNthItemn :
	beq $t2, $t1, doneGoNthItemn
	beq $t6, $0, Insert_end # If next is null before 
	...
	add $t6, $0, $v0 #Pointer to allocated memory
	sw $t6, 4($t0) #next to the last one
	sw $0, 0($t6) #put NULL
	sw $t1, 4($t6) #put value
	add $v0, $0, 0 #success
...
Delete_x:
	...
	lw $t3, 4($s0)
	bne $t3, $t1 , notHeadx #Delete first one so head
...
sum: 
	...
	bne $s0, $zero, devam80sum	# if pointer is NULL, there is no list
	
	la $a0, msg135	# put msg82 address into a0
	li $v0, 4	# system call to print
	syscall		#   out the msg82 string

	j Return80sum	# done, so go home

devam80sum:		# top of loop	
...
Top80sum:
	beq $s1, $zero, Return80sum # if pointerToNext is NULL, there are no more elements
	...
	j Top80sum		# go back to top of loop, to test and
			#   possibly iterate again
...
Return80sum:	
	...
	la $a0, msg85	# put msg134 address into a0
	li $v0, 4	# system call to print
	syscall		# out the msg134 string
...
msg118:  .asciiz "8 - exit this program \n"
...
msg133:  .asciiz "7 - print the sum \n"
msg134:  .asciiz "The sum is "
msg135:  .asciiz "The sum is 0 \n"
